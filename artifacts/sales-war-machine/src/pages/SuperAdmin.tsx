...
type Org = {
  ...
  welcomeEmailSentAt: string | null; trialNudgeSentAt: string | null;
  createdAt: string;
};
...
type Tab = "users" | "overview" | "organizations" | "waitlist" | "feedback";
...
export default function SuperAdmin({ initialTab }: { initialTab?: Tab } = {}) {
  const qc = useQueryClient();
  const [tab, setTab] = useState<Tab>(initialTab ?? "users");
  const [userSearch, setUserSearch] = useState("");
  const [orgEmailFilter, setOrgEmailFilter] = useState<"all" | "no_welcome" | "no_nudge" | "no_either">("all");
  ...
  const { data: saasUsers, isLoading: usersLoading } = useQuery<SaasUser[]>({
    queryKey: ["saas-users"],
  ...
  const { data: accessLogData } = useQuery<AccessLogEntry[]>({
    queryKey: ["saas-access-log"],
    queryFn: async () => {
      const res = await fetch(`/api/saas/access-log`, { credentials: "include" });
      if (!res.ok) return [];
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-1 p-1 rounded-xl w-fit flex-wrap" style={{ background: "#F3F4F6" }}>
        {(["users", "overview", "organizations", "waitlist", "feedback"] as Tab[]).map(t => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === "users" ? `Users${saasUsers ? ` (${saasUsers.length})` : ""}` : t}
            {t === "waitlist" && data?.summary?.pendingInvite ? ` (${data.summary.pendingInvite})` : ""}
      ...
      {tab === "users" && (
        <div className="rounded-2xl border overflow-hidden" style={{ borderColor: "hsl(220 13% 91%)" }}>
          {/* Header */}
          <div className="px-5 py-4 border-b flex items-center justify-between gap-4 flex-wrap" style={{ borderColor: "hsl(220 13% 91%)" }}>
            <div className="flex items-center gap-2">
              <Users className="w-4 h-4 text-violet-500" />
              <span className="text-sm font-bold text-gray-800">
                {usersLoading ? "Loading users…" : `${saasUsers?.length ?? 0} Registered Users`}
              </span>
            </div>
            <input
              type="text"
              ...
              className="border rounded-lg px-3 py-1.5 text-xs w-64 focus:outline-none focus:ring-2"
              style={{ borderColor: "hsl(220 13% 88%)", focusRingColor: "#7C3AED" } as React.CSSProperties}
            />
          </div>

          {usersLoading ? (
            <div className="p-10 text-center"><Loader2 className="w-5 h-5 animate-spin text-violet-500 mx-auto" /></div>
          ) : !saasUsers?.length ? (
            <div className="p-10 text-center text-sm text-gray-400">No users registered yet.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr style={{ background: "#F9FAFB" }}>
                    {["User", "Email", "Company", "Phone", "Website", "Plan", "Status", "Role", "Verified", "Joined"].map(h => (
                  ...
                  {(saasUsers ?? [])
                    .filter(u => {
                      ...
                      return (
                        ...
                        (u.orgPhone ?? "").toLowerCase().includes(q)
                      );
                    })
                    .map(u => {
                      const plan = u.orgPlan ?? "—";
                      const status = u.orgSubscriptionStatus ?? "—";
                      const statusStyle = STATUS_COLORS[status] ?? STATUS_COLORS.canceled!;
                      return (
                        <tr key={u.id} className="border-t hover:bg-gray-50 transition-colors" style={{ borderColor: "hsl(220 13% 93%)" }}>
                          {/* User */}
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <div className="text-xs font-bold text-gray-900 whitespace-nowrap">{u.firstName} {u.lastName}</div>
                                {!u.isActive && <div className="text-[10px] text-red-500 font-semibold">Inactive</div>}
                              </div>
                            </div>
                          </td>
                          {/* Email */}
                          ...
                          {/* Company */}
                          <td className="px-4 py-3">
                            <div className="text-xs font-semibold text-gray-800 whitespace-nowrap">{u.orgName ?? "—"}</div>
                            {u.orgEmail && u.orgEmail !== u.email && (
                              <div className="text-[10px] text-gray-400">{u.orgEmail}</div>
                            )}
                          </td>
                          {/* Phone */}
                          <td className="px-4 py-3">
                            <div className="text-[11px] text-gray-600 whitespace-nowrap">{u.orgPhone ?? "—"}</div>
                          </td>
                          {/* Website */}
                          <td className="px-4 py-3">
                            {u.orgWebsite ? (
                              <a href={u.orgWebsite} target="_blank" rel="noopener noreferrer"
                                className="text-[11px] text-violet-600 hover:underline flex items-center gap-1 whitespace-nowrap">
                          ...
                          <td className="px-4 py-3">
                            <span className="text-[11px] font-bold px-2 py-0.5 rounded-full capitalize"
                              style={{ background: (PLAN_COLORS[plan] ?? "#9CA3AF") + "18", color: PLAN_COLORS[plan] ?? "#9CA3AF" }}>
                              {plan}
                            </span>
                          </td>
                          {/* Subscription Status */}
                          ...
                          {/* Role */}
                          <td className="px-4 py-3">
                            <span className="text-[11px] font-semibold capitalize text-gray-600">{u.role}</span>
                          </td>
                          {/* Verified */}
                          <td className="px-4 py-3">
                            {u.isVerified ? (
                          ...
                          {/* Joined */}
                          <td className="px-4 py-3">
                            <div className="text-[11px] text-gray-500 whitespace-nowrap">{fmtDate(u.createdAt)}</div>
                            <div className="text-[10px] text-gray-400">{fmtRelative(u.createdAt)}</div>
                          </td>
                        </tr>
                      );
                    })}
      ...
      {tab === "overview" && (
        <>
          ...
          <div className="rounded-2xl border overflow-hidden" style={{ borderColor: "hsl(220 13% 91%)" }}>
            <div className="px-5 py-3 border-b flex items-center justify-between" style={{ borderColor: "hsl(220 13% 91%)" }}>
              <div className="flex items-center gap-2">
                <Shield className="w-4 h-4 text-gray-400" />
                <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Recent Admin Access</span>
              </div>
              <div className="flex items-center gap-3">
                {accessLogData && accessLogData.filter(e => e.status === "forbidden").length > 0 && (
                  <span className="flex items-center gap-1 text-[10px] font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded-full">
                    <ShieldAlert className="w-3 h-3" />
                    {accessLogData.filter(e => e.status === "forbidden").length} rejected
                  </span>
                )}
                <button
                  onClick={() => qc.invalidateQueries({ queryKey: ["saas-access-log"] })}
            ...
            {!accessLogData ? (
              <div className="p-6 text-center"><Loader2 className="w-4 h-4 animate-spin text-violet-400 mx-auto" /></div>
            ) : accessLogData.length === 0 ? (
              <div className="px-5 py-6 text-center text-sm text-gray-400">No access events recorded yet.</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr style={{ background: "#F9FAFB" }}>
                      {["Status", "Method", "Path", "IP", "When"].map(h => (
                        <th key={h} className="text-left px-4 py-2.5 text-[10px] font-bold uppercase tracking-wide text-gray-400">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {accessLogData.slice(0, 50).map((entry, i) => (
                      <tr
                        key={i}
                        className="border-t hover:bg-gray-50 transition-colors"
                        style={{ borderColor: "hsl(220 13% 93%)", background: entry.status === "forbidden" ? "#FFF8F8" : undefined }}
                      >
                        <td className="px-4 py-2.5">
                          {entry.status === "success" ? (
                            <span className="flex items-center gap-1 text-[10px] font-bold text-green-700 bg-green-50 px-2 py-0.5 rounded-full w-fit">
                              <ShieldCheck className="w-3 h-3" /> Allowed
                            </span>
                          ) : (
                            <span className="flex items-center gap-1 text-[10px] font-bold text-red-700 bg-red-50 px-2 py-0.5 rounded-full w-fit">
                              <ShieldAlert className="w-3 h-3" /> Rejected{entry.secret ? ` (${entry.secret})` : ""}
                            </span>
                          )}
                        </td>
                        <td className="px-4 py-2.5">
                          <span className="text-[10px] font-bold font-mono text-gray-600">{entry.method}</span>
                        </td>
                        <td className="px-4 py-2.5 max-w-xs">
                          <span className="text-[11px] font-mono text-gray-700 truncate block" title={entry.path}>{entry.path}</span>
                        </td>
                        <td className="px-4 py-2.5">
                          <span className="text-[11px] font-mono text-gray-500">{entry.ip}</span>
                        </td>
                        <td className="px-4 py-2.5">
                          <span className="text-[11px] text-gray-400 whitespace-nowrap">{fmtRelative(entry.timestamp)}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
      ...
      {tab === "organizations" && (
        <div className="rounded-2xl border overflow-hidden" style={{ borderColor: "hsl(220 13% 91%)" }}>
          <div className="px-5 py-4 border-b flex items-center justify-between flex-wrap gap-3" style={{ borderColor: "hsl(220 13% 91%)" }}>
            <span className="text-sm font-bold text-gray-800">{orgs?.length ?? 0} Organizations</span>
            <div className="flex items-center gap-2 flex-wrap">
              {(["all", "no_welcome", "no_nudge", "no_either"] as const).map(f => {
                const labels: Record<typeof f, string> = {
                  all: "All",
                  no_welcome: "No welcome email",
                  no_nudge: "No nudge email",
                  no_either: "Missing either email",
                };
                return (
                  <button key={f} onClick={() => setOrgEmailFilter(f)}
                    className="text-[11px] font-semibold px-3 py-1 rounded-full transition-all"
                    style={orgEmailFilter === f
                    ...
                    {f !== "all" && orgs && (
                      <span className="ml-1 opacity-70">
                        ({orgs.filter(o =>
                          f === "no_welcome" ? !o.welcomeEmailSentAt :
                          f === "no_nudge"   ? !o.trialNudgeSentAt :
                          !o.welcomeEmailSentAt || !o.trialNudgeSentAt
                        ).length})
                      </span>
                    )}
                  </button>
              ...
              <button onClick={() => qc.invalidateQueries({ queryKey: ["saas-orgs"] })} className="text-gray-400 hover:text-gray-600 ml-1">
          ...
          {!orgs ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr style={{ background: "#F9FAFB" }}>
                    {["Organization", "Plan", "Status", "Usage", "Trial/Renewal", "Emails", "Actions"].map(h => (
                      <th key={h} className="text-left px-4 py-3 text-[11px] font-bold uppercase tracking-wide text-gray-400 whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {orgs.filter(org =>
                    orgEmailFilter === "all"        ? true :
                    orgEmailFilter === "no_welcome" ? !org.welcomeEmailSentAt :
                    orgEmailFilter === "no_nudge"   ? !org.trialNudgeSentAt :
                    !org.welcomeEmailSentAt || !org.trialNudgeSentAt
                  ).map(org => {
                      ...
                      <tr key={org.id} className="border-t hover:bg-gray-50 transition-colors" style={{ borderColor: "hsl(220 13% 93%)" }}>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex flex-col gap-1">
                            <div className="flex items-center gap-1" title={org.welcomeEmailSentAt ? `Sent ${fmtDate(org.welcomeEmailSentAt)}` : "Not sent"}>
                              {org.welcomeEmailSentAt
                                ? <CheckCircle2 className="w-3.5 h-3.5 text-green-500 flex-shrink-0" />
                                : <XCircle className="w-3.5 h-3.5 text-red-400 flex-shrink-0" />}
                              <span className="text-[10px] text-gray-500 whitespace-nowrap">Welcome{org.welcomeEmailSentAt ? ` ${fmtDate(org.welcomeEmailSentAt)}` : ""}</span>
                            </div>
                            <div className="flex items-center gap-1" title={org.trialNudgeSentAt ? `Sent ${fmtDate(org.trialNudgeSentAt)}` : "Not sent"}>
                              {org.trialNudgeSentAt
                                ? <CheckCircle2 className="w-3.5 h-3.5 text-green-500 flex-shrink-0" />
                                : <Clock className="w-3.5 h-3.5 text-gray-300 flex-shrink-0" />}
                              <span className="text-[10px] text-gray-500 whitespace-nowrap">Nudge{org.trialNudgeSentAt ? ` ${fmtDate(org.trialNudgeSentAt)}` : ""}</span>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-3">

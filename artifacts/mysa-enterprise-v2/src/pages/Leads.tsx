...
export default function Leads() {
  ...
  const exportCsv = () => {
    ...
    const headers = ["ID", "First Name", "Last Name", "Email", "Company", "Designation", "Industry", "Country", "Status", "BANT Score", "Source", "Created"];
    const csvRows = rows.map((l) => [
      l.id, l.firstName, l.lastName, l.email, l.company, l.designation ?? "", l.industry ?? "", l.country ?? "", l.status, l.bantScore ?? "", l.source ?? "", l.createdAt ?? "",
    ...
    <div className="p-3 md:p-6 space-y-3 md:space-y-4">
      ...
      <div className="space-y-2" style={{ display: viewTab === "leads" ? undefined : "none" }}>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={exportCsv} className="flex items-center gap-1 px-2.5 py-1.5 text-xs rounded border border-gray-200 bg-gray-50 text-muted-foreground hover:text-gray-900 ml-auto">
        ...
        <div className="flex gap-1 flex-wrap">
          {STATUSES.map((s) => (
            <button
              ...
              className={cn("px-2.5 py-1 text-[11px] rounded border transition-colors capitalize", s === status ? "border-teal-300 text-teal-700 bg-teal-50" : "border-gray-200 text-muted-foreground hover:text-gray-900 bg-gray-50")}
            >
              {STATUS_DISPLAY[s] ?? s}
            </button>
          ))}
      ...
      {selected.length > 0 && viewTab === "leads" && (
        <div className="flex items-center gap-3 px-3 py-2 rounded border border-amber-200 bg-amber-50 flex-wrap">
          ...
          <button onClick={exportCsv} className="flex items-center gap-1 text-xs text-blue-700 hover:text-blue-200">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {bantScore != null ? (
                        <div className="flex-shrink-0 text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ background: scoreColor + "18", color: scoreColor }}>
                          {bantScore}
                        </div>
                    ...
                    <div className="mt-2 flex justify-end" onClick={e => e.stopPropagation()}>
                      <button
                        title={lead.whatsapp || lead.phone ? "Initiate WhatsApp" : "No phone number"}
                        disabled={(!lead.whatsapp && !lead.phone) || loadingLeads.has(lead.id)}
                        onClick={() => {
                          ...
                          initiateWa.mutate(
                            ...
                            {
                              onSuccess: (d) => {
                                toast({ title: "WhatsApp hook sent", description: d.message ?? `Conversation started with ${lead.firstName}` });
                              },
                              onError: () => {
                                toast({ title: "WhatsApp failed", description: "Could not initiate", variant: "destructive" });
                              },
                              onSettled: () => {
                                setLoadingLeads(prev => { const next = new Set(prev); next.delete(lead.id); return next; });
                              },
                            }
                          );
                        }}
                        className={cn(
                          ...
                          (lead.whatsapp || lead.phone) && !loadingLeads.has(lead.id)
            ...
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                <th className="sticky left-0 z-20 bg-gray-50 px-2 py-2.5">
                  <input type="checkbox" onChange={selectAll} className="w-3.5 h-3.5 accent-teal-500" />
                </th>
                ...
                >
                  ...
                  <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        title={lead.whatsapp || lead.phone ? "Initiate WhatsApp" : "No phone number — add one to initiate"}
                        disabled={(!lead.whatsapp && !lead.phone) || loadingLeads.has(lead.id)}
                        onClick={() => {
                          setLoadingLeads(prev => new Set([...prev, lead.id]));
                          initiateWa.mutate(
                            { leadId: lead.id },
                            {
                              ...
                              onError: () => {
                                toast({ title: "WhatsApp failed", description: "Could not initiate conversation", variant: "destructive" });
                              },
                              onSettled: () => {
                                setLoadingLeads(prev => { const next = new Set(prev); next.delete(lead.id); return next; });
                              },
                      ...
                      >
                        {loadingLeads.has(lead.id)
                          ...
                          : <MessageCircle className={cn("w-3.5 h-3.5", (lead.whatsapp || lead.phone) ? "text-gray-300 hover:text-green-600" : "text-gray-200 cursor-not-allowed")} />}
                      </button>
                  ...
                  <>
                    <div className="flex items-end gap-3 mb-4">
                      <div
                        ...
                        style={{ color: bantBandDarkColor(lead.bantScore, D.accent) }}
                      >
                        {lead.bantScore}
                      </div>
                      <div className="pb-1">
                        <div className="text-xs font-bold" style={{ color: bantBandDarkColor(lead.bantScore, D.accent) }}>
                    ...
                    <div className="relative h-1.5 rounded-full mb-4 overflow-hidden" style={{ background: D.cardBorder }}>
                      <div
                        ...
                        style={{
                          ...
                          background: bantBandDarkGradient(lead.bantScore, D.accent),
                          boxShadow: `0 0 8px ${bantBandDarkColor(lead.bantScore, D.accent)}66`,

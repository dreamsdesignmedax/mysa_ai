...
function SidebarUsageMeter() {
  ...
  const { usage } = data;
  const metrics: { label: string; used: number; max: number }[] = [
  ...
  return (
    <Link href="/billing">
      <div
        ...
        style={{ background: "hsl(220 13% 96%)", border: "1px solid hsl(220 13% 91%)" }}
        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = "#C4B5FD"; }}
        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = "hsl(220 13% 91%)"; }}
      >
        <div className="flex items-center justify-between mb-2">
          <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: "#9CA3AF" }}>Usage this month</span>
          <ArrowRight className="w-3 h-3 flex-shrink-0" style={{ color: "#C4B5FD" }} />
        </div>

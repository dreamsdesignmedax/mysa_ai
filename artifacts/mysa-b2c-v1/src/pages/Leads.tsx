...
export default function Leads() {
  ...
  const [bantBatchResult, setBantBatchResult] = useState<{ scored: number } | null>(null);
  const [enrichBatchResult, setEnrichBatchResult] = useState<{ enriched: number } | null>(null);
  ...
  const bulkUpdate = useBulkUpdateLeads({
    mutation: {
      ...
      onError: (err) => {
        toast({ title: "Bulk update failed", description: (err as Error).message ?? "Could not update leads. Please try again.", variant: "destructive" });
      },
    },
  });
  const importPaste = useImportLeadsPaste({
    mutation: {
      ...
      onError: (err) => {
        setImporting(false);
        toast({ title: "Import failed", description: (err as Error).message ?? "Could not import leads. Please try again.", variant: "destructive" });
      },
    },
  });
  ...
  const handleCsvImport = async () => {
    ...
    for (let i = 0; i < chunks.length; i++) {
      if (planLimitHit) { totalSkipped += chunks[i].length; continue; }
      ...
      {selected.length > 0 && (
        <div className="flex-shrink-0 flex items-center gap-2 px-5 py-2 border-b border-violet-200 bg-violet-50 flex-wrap">
          ...
          <button disabled={enrichBatchPoller.isPolling} onClick={async () => {
            if (!confirm(`Enrich AI keywords for ${selected.length} lead(s)? Results arrive asynchronously via Anthropic Batch API.`)) return;
            try {
              ...
              const result = await r.json() as { batchId?: string; leadsCount?: number; error?: string };
              if (!r.ok || !result.batchId) { toast({ title: result.error ?? "Enrichment submission failed", variant: "destructive" }); return; }
          ...
          {enrichBatchResult && !enrichBatchPoller.isPolling && (
            <span className="text-[11px] text-teal-600 font-medium">✓ {enrichBatchResult.enriched} enriched</span>
          )}
          {sequences.length > 0 && (
          ...
          <button disabled={bantBatchPoller.isPolling} onClick={async () => {
            if (!confirm(`Submit BANTB async scoring for ${selected.length} lead(s)? Results arrive via Anthropic Batch API (~30s–few min).`)) return;
            try {
              ...
              const result = await r.json() as { batchId?: string; leadsCount?: number; error?: string };
              if (!r.ok || !result.batchId) { toast({ title: result.error ?? "BANT batch submission failed", variant: "destructive" }); return; }
            ...
            } catch { toast({ title: "BANT batch failed", variant: "destructive" }); }
          ...
          {bantBatchResult && !bantBatchPoller.isPolling && (
            <span className="text-[11px] text-violet-600 font-medium">✓ {bantBatchResult.scored} scored</span>
          )}

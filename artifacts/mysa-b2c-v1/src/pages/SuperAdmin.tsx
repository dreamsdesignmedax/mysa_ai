...
type Tab = "overview" | "organizations" | "waitlist" | "feedback" | "api-usage";

type ApiUsageData = {
  summary: { totalCostUsd: number; totalInputTokens: number; totalOutputTokens: number; totalApiCalls: number };
  daily: Array<{ date: string; service: string; totalCostUsd: number; totalCalls: number }>;
  perOrgToday: Array<{
    id: number; name: string; email: string;
    limitUsd: number | null; todaySpend: number; alertSentAt: string | null;
  }>;
  platformStats: { auditsDone: number; reportsGenerated: number; leadsFetched: number; leadBankSize: number; leadsEnrichedViaApi: number };
};
...
export default function SuperAdmin() {
  ...
  const { data: apiUsageData, isLoading: apiUsageLoading } = useQuery<ApiUsageData>({
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-1 p-1 rounded-xl w-fit flex-wrap" style={{ background: "#F3F4F6" }}>
        {(["overview", "organizations", "waitlist", "feedback", "api-usage"] as Tab[]).map(t => (
      ...
      {tab === "api-usage" && (
        <div className="space-y-6">
          {apiUsageLoading ? (
            <div className="flex items-center justify-center py-16"><Loader2 className="w-5 h-5 animate-spin text-violet-500" /></div>
          ) : apiUsageData ? (
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  <div key={label} className="rounded-2xl border px-4 py-4" style={{ background: bg, borderColor: "hsl(220 13% 91%)" }}>
              ...
              <div className="rounded-2xl border overflow-hidden" style={{ borderColor: "hsl(220 13% 91%)" }}>
                ...
                <div className="divide-y" style={{ borderColor: "hsl(220 13% 93%)" }}>
                        ...
                        <div key={org.id} className="px-5 py-3">
                          <div className="flex items-center justify-between mb-1.5">
                            <div className="flex items-center gap-2 min-w-0">
                              ...
                              {isOver && (
                                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-red-100 text-red-600 flex-shrink-0">EXCEEDED</span>
                              )}
                              {isWarning && (
                                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-700 flex-shrink-0">NEARING LIMIT</span>
                              )}
                            </div>
                            <div className="text-right flex-shrink-0 ml-4">
                              ...
                              {org.limitUsd && (
                                <span className="text-xs text-gray-400"> / ${org.limitUsd.toFixed(2)}</span>
                              )}
                              {!org.limitUsd && (
                                <span className="text-xs text-gray-300 ml-1">no limit</span>
                              )}
                            </div>
                          </div>
                          {org.limitUsd && (
                  ...
                  {apiUsageData.perOrgToday.filter(o => o.todaySpend > 0 || o.limitUsd !== null).length === 0 && (

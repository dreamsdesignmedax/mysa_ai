  ...
  const generateSingle = useCallback((leadId: number) => {
    ...
    es.addEventListener("error", (e) => {
      try {
        ...
        if (planLimit) dispatchPlanLimitEvent(planLimit);
        setGenerating({ leadId, status: "error", message: d.message as string ?? "Generation failed" });
      } catch {
  ...
  const generateAll = useCallback(() => {
    ...
    es.addEventListener("error", (e) => {
      try {
        ...
        const planLimit = tryParsePlanLimit(d);
        if (planLimit) dispatchPlanLimitEvent(planLimit);
        setBulkMessage((d.message as string | undefined) ?? "Bulk generation encountered an error");
      } catch {

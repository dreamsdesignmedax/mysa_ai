...
import { ArrowLeft, ChevronLeft, ChevronRight, Globe, Linkedin, Phone, Mail, Building2, MapPin, MessageCircle, Pencil, X, Check, Loader2, Sparkles, CalendarPlus, CalendarDays, Download, UserCheck } from "lucide-react";
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-gray-200 p-4 bg-white shadow-sm">
            ...
            {(lead as unknown as { assignedToName?: string }).assignedToName ? (
              <div className="flex items-center justify-between gap-2 mb-2">
                <span className="text-xs font-semibold text-indigo-700 bg-indigo-50 px-2 py-1 rounded">{(lead as unknown as { assignedToName: string }).assignedToName}</span>
              </div>
            ) : (
              <p className="text-xs text-gray-400 mb-2">No rep assigned yet</p>
            )}
            {canReassign && (
              <select
                value={(lead as unknown as { assignedToId?: number }).assignedToId ?? ""}
                onChange={(e) => {
                  ...
                  const displayName = selected
                    ...
                    : null;
                  updateLead.mutate(
                    { id, data: { assignedToId: val ? Number(val) : null } as Parameters<typeof updateLead.mutate>[0]["data"] },
                    {
                      onSuccess: () => {
                        toast({

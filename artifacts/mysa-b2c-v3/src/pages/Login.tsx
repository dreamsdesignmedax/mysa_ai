...
import { Eye, EyeOff, ArrowRight, Phone, ChevronLeft } from "lucide-react";
import {
  RecaptchaVerifier,
...
export default function Login({ onSuccess, onRegisterClick }: LoginProps) {
  ...
  /* ── Profile modal (new users after OTP) ── */
  ...
  async function handleProfileSave(e: FormEvent) {
    ...
    if (!profEmail.trim())     { setProfError("Email address is required."); return; }
    setProfLoading(true);
    try {
      const body: Record<string, string> = { firstName: profFirstName.trim(), email: profEmail.trim() };
      ...
      if (!res.ok) {
        ...
        setProfError((data as { error?: string }).error ?? "Couldn't save your profile. Please try again.");
        return;
      }
  ...
  function resetPhoneFlow() {
    setPhoneStep("idle"); setPhoneNumber(""); setOtpCode(""); setPhoneError(""); setResendSecs(0);
  ...
  return (
    ...
    >
      ...
      <div className="lg:hidden absolute inset-0 pointer-events-none">
        ...
        <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse 380px 300px at 15% 20%, rgba(167,139,250,0.22) 0%, transparent 70%), radial-gradient(ellipse 280px 220px at 85% 75%, rgba(109,40,217,0.28) 0%, transparent 70%)" }} />
        <div style={{ position: "absolute", top: "6%", right: "8%", width: 130, height: 130, borderRadius: "50%", background: "radial-gradient(circle, rgba(167,139,250,0.14) 0%, transparent 70%)", filter: "blur(22px)" }} />
        <div style={{ position: "absolute", bottom: "22%", left: "4%", width: 160, height: 160, borderRadius: "50%", background: "radial-gradient(circle, rgba(124,58,237,0.18) 0%, transparent 70%)", filter: "blur(26px)" }} />
      </div>

      {/* ── Desktop left panel ── */}
      <div
        className="hidden lg:flex flex-col justify-between w-[480px] flex-shrink-0 px-14 py-12 relative overflow-hidden"
      ...
      >
        ...
        <div className="relative z-10">
          <div style={{ display: "inline-flex", alignItems: "center", fontSize: 11, fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", color: "#A78BFA", background: "rgba(167,139,250,0.12)", border: "1px solid rgba(167,139,250,0.2)", padding: "5px 14px", borderRadius: 100, marginBottom: 24 }}>
            AI Sales Automation
          </div>
          <h2 style={{ fontSize: "clamp(28px,2.5vw,38px)", fontWeight: 800, color: "white", lineHeight: 1.15, letterSpacing: "-0.025em", marginBottom: 16 }}>
            The AI Sales Brain for{" "}
            <span style={{ color: "#C4B5FD", textShadow: "0 0 40px rgba(167,139,250,0.4)" }}>B2B Growth Companies</span>
          </h2>
          <p style={{ fontSize: 15, lineHeight: 1.7, color: "rgba(255,255,255,0.55)", marginBottom: 40 }}>
            The only sales OS that audits your prospects, scores their belief, writes personalised outreach, and closes deals.
          </p>
          <div className="grid grid-cols-3 gap-3">
            {[{ val: "3×", label: "Pipeline growth" }, { val: "68%", label: "Faster outreach" }, { val: "41%", label: "More deals won" }].map(({ val, label }) => (
              <div key={label} className="rounded-2xl p-4 text-center" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(196,181,253,0.15)" }}>
                <div style={{ fontSize: 26, fontWeight: 900, color: "white", letterSpacing: "-0.025em", lineHeight: 1 }}>{val}</div>
                <div style={{ fontSize: 11, marginTop: 5, color: "rgba(167,139,250,0.65)", fontWeight: 500 }}>{label}</div>
              </div>
            ))}
          </div>
        </div>

        <p style={{ fontSize: 11, color: "rgba(255,255,255,0.22)", position: "relative", zIndex: 10 }}>© {new Date().getFullYear()} Dreamsdesign · MysaAI</p>
      </div>

      {/* ── Right / mobile panel ── */}
      <div className="flex-1 flex flex-col lg:bg-[#F5F3FF] relative z-10">

        {/* Mobile hero — idle only */}
        {phoneStep === "idle" && (
          <div className="lg:hidden flex flex-col items-center text-center px-6 pt-16 pb-8">
            ...
            <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "3px", textTransform: "uppercase", color: "rgba(167,139,250,0.7)", marginBottom: 6 }}>MysaAI</div>

            <div style={{ display: "inline-flex", alignItems: "center", fontSize: 10, fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", color: "#A78BFA", background: "rgba(167,139,250,0.1)", border: "1px solid rgba(167,139,250,0.2)", padding: "4px 12px", borderRadius: 100, marginBottom: 20 }}>
              AI Sales Automation
            </div>

            <h1 style={{ fontSize: 30, fontWeight: 900, color: "white", lineHeight: 1.12, letterSpacing: "-0.035em", marginBottom: 10 }}>
              Welcome back.<br />
              <span style={{ color: "#C4B5FD", textShadow: "0 0 30px rgba(167,139,250,0.35)" }}>Let's close deals.</span>
            </h1>

            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.5)", lineHeight: 1.65, maxWidth: 270, marginBottom: 24 }}>
              Sign in with your mobile number to pick up right where you left off.
            </p>
        ...
        {/* Form area */}
        <div className={`flex-1 flex flex-col items-center ${phoneStep === "idle" ? "justify-start lg:justify-center" : "justify-center"} px-5 pb-10 lg:py-12`}>

          {/* Desktop-only title */}
          {phoneStep === "idle" && (
            <div className="hidden lg:block w-full max-w-[400px] mb-8">
              <h1 style={{ fontSize: 28, fontWeight: 800, color: "#111827", letterSpacing: "-0.025em", lineHeight: 1.15, marginBottom: 4 }}>Welcome back</h1>
              <p style={{ fontSize: 14, color: "#6B7280" }}>Sign in to your MysaAI account</p>
            </div>
          )}
          ...
          <div
            ...
            style={{ background: "white", borderRadius: 24, boxShadow: "0 8px 48px rgba(0,0,0,0.28), 0 2px 12px rgba(0,0,0,0.12)", padding: "28px 22px" }}
          >
            ...
            {phoneStep !== "idle" ? (
                    ...
                    <form onSubmit={handleVerifyOtp} className="space-y-4">
                      ...
                      <div style={{ textAlign: "center", fontSize: 13, color: "#6B7280" }}>
                        {resendSecs > 0 ? <span>Resend in {resendSecs}s</span> : (
                          <button type="button" onClick={() => { setPhoneStep("phone"); setOtpCode(""); setPhoneError(""); }}
              ...
              /* ── Idle: primary options ── */
              <div className="space-y-3">
                <p className="lg:hidden text-center text-xs text-gray-400 font-medium pb-1">Sign in to your account</p>

                {/* ── Continue with Mobile — PRIMARY ── */}
                <button type="button"
                  onClick={() => {
                    if (otpEnabled) { setPhoneStep("phone"); setPhoneError(""); setOtpCode(""); }
                ...
                {/* Email/password — admin fallback, hidden by default */}
                <div className="pt-1">
                  {!showEmailForm ? (
                    <button type="button" onClick={() => setShowEmailForm(true)}
                      className="w-full text-center transition-colors"
                      style={{ background: "none", border: "none", cursor: "pointer", fontSize: 12, color: "#9CA3AF", fontWeight: 500, padding: "6px 0" }}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-3 pt-1">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <span style={{ fontSize: 11, color: "#9CA3AF", fontWeight: 500 }}>email sign-in</span>
                        <div className="flex-1 h-px" style={{ background: "#F0EDFF" }} />
                      </div>
                      <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                        required autoComplete="email" placeholder="Email address"
                        className={inputBase} style={{ border: "1.5px solid #EDE9FE" }} />
                      <div className="relative">
                        <input type={showPw ? "text" : "password"} value={password}
                          onChange={e => setPassword(e.target.value)} required
                          autoComplete="current-password" placeholder="Password"
                          className={inputBase + " pr-10"} style={{ border: "1.5px solid #EDE9FE" }} />
                      ...
                      <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 py-3 rounded-xl transition-all"
                        style={{ fontSize: 14, fontWeight: 700, color: "white", border: "none", background: loading ? "#9CA3AF" : "linear-gradient(135deg, #2D1B69, #7C3AED)", cursor: loading ? "not-allowed" : "pointer", boxShadow: loading ? "none" : "0 4px 14px rgba(109,40,217,0.3)" }}>
          ...
          {/* Sign up + terms — below card */}
          {phoneStep === "idle" && (
            <div className="mt-6 text-center w-full max-w-[400px]">
              <p style={{ fontSize: 13, marginBottom: 12 }}>
                <span className="text-white/60 lg:text-gray-500">Don't have an account? </span>
              ...
              <p style={{ fontSize: 11, lineHeight: 1.7 }}>
                <span className="text-white/30 lg:text-gray-400">By signing in, you agree to our </span>
      ...
      {/* ── Complete your profile modal — email is REQUIRED ── */}
      {showProfileModal && (
            ...
            {!profEmailSent ? (
              <>
                ...
                <form onSubmit={handleProfileSave} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  ...
                  {/* Row 2: Email (required) + Phone (read-only) */}
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    <div>
                      <label style={{ display: "block", fontSize: 11, fontWeight: 600, marginBottom: 5, color: "#374151" }}>
                        Work email <span style={{ color: "#7C3AED" }}>*</span>
                      </label>
                      <input type="email" value={profEmail} onChange={e => setProfEmail(e.target.value)} placeholder="you@company.com" required className={inputSm} style={{ border: "1.5px solid #EDE9FE" }} />
                    </div>
                    <div>
